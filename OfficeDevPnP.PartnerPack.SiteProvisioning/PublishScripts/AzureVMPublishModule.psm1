#  AzureVMPublishModule.psm1 is a Windows PowerShell script module. This module exports Windows PowerShell functions that automate life cycle management for web applications. You can use the functions as is or customize them for your application and publishing environment.
...
<#
.SYNOPSIS
Finds a Microsoft Azure storage account named "devtest*" in the current subscription.
...
.PARAMETER  Location
Specifies the location of the storage account. Valid values are the Microsoft Azure locations, such as "West US". You can enter a location or an affinity group, but not both.
...
<#
.SYNOPSIS
Creates a new Microsoft Azure storage account with a unique name that begins with "devtest".

.DESCRIPTION
The Add-AzureVMStorage function creates a new Microsoft Azure storage account in the current subscription. The name of the account begins with "devtest" followed by a unique alphanumeric string. The function returns the name of the new storage account. You must specify either a location or an affinity group for the new storage account.
...
function Add-AzureVMStorage
{
    ...
    # Create a new Microsoft Azure storage account with location/affinity group
...
function Read-ConfigFile
{
    ...
    # Determine whether the environmentSettings object has 'cloudService' properties (regardless of the property value)
    $hasCloudServiceProperty = Test-Member -Object $config.environmentSettings -Member 'cloudService'

    if (!$hasCloudServiceProperty)
    {
        throw 'Read-ConfigFile: Configuration file does not contain a cloudService property.'
    }
...
<#
.SYNOPSIS
Creates all elements of a new virtual machine in a Microsoft Azure subscription.

.DESCRIPTION
This function creates a Microsoft Azure virtual machine (VM) and returns the URL of the deployed VM. The function sets up the prerequisites and then calls the New-AzureVM cmdlet (Azure module) to create a new VM. 
...
-- It calls the Add-AzureVM cmdlet to create a new Microsoft Azure virtual machine and returns the new VM. The values of the function parameters are typically taken from the JSON configuration file that Visual Studio generates for Microsoft Azure-integrated web projects. The Read-ConfigFile function in this module converts the JSON file into a hash table. Save the cloudservice key of the hash table in a variable (as a PSCustomObject), and uses the properties of the custom object as parameter values.
...
Specifies an existing Microsoft Azure service or a name of a new Microsoft Azure service. This value is submitted to the ServiceName parameter of the New-AzureVM cmdlet, which adds the new virtual machine to an exiting Microsoft Azure service, or, if Location or AffinityGroup is specified, creates a new virtual machine and service in the current subscription. This parameter is required. 
...
Specifies the name of an availability set for the new VM. When you place multiple virtual machines in an availability set, Microsoft Azure tries to keep those virtual machines on separate hosts to improve continuity of service should one fail. This parameter is optional. 
...
.PARAMETER  Location
Specifies a location for the new VM. Valid values are the Microsoft Azure locations, such as "West US". The default is the location of the subscription. This parameter is optional. 

.PARAMETER  AffinityGroup
Specifies an affinity group for the new VM. An affinity group is a group of related resources. When you specify an affinity group, Microsoft Azure tries to keep the resources in the group together to improve efficiency. 
...
Prepares the VM to for deployment. Prepares the VM to for deployment. This parameter is optional. If it is not specified, the VM is created, but it is not deployed. The value of this parameter is included in the JSON configuration file that Visual Studio generates for cloud services.

.PARAMETER VMImage
Specifies that the ImageName is the name of a VMImage and not an OSImage. This parameter is optional. If it is not specified, ImageName will be treated as an OSImage. The value of this parameter is included in the JSON configuration file that Visual Studio generates for virtual machines.

.PARAMETER GeneralizedImage
For a VMImage specifies if the OS state is generalized. This parameter is optional. If it is not specified, the script behaves as it would for a specialized VMImage. This parameter is ignored for OSImages.  The value of this parameter is included in the JSON configuration file that Visual Studio generates for virtual machines.
...
function Add-AzureVM
{
    ...
    # Create a new Microsoft Azure VM configuration object.
    ...
    # VMImages do not support location for now - the new VM will be created in the same storage account(location) where the image exists
...
<#
.SYNOPSIS
Gets the specified Microsoft Azure virtual machine.

.DESCRIPTION
The Find-AzureVM function gets a Microsoft Azure virtual machine (VM) based on the service name and VM name. This function calls the Test-AzureName cmdlet (Azure module) to verify that the service name exists in Microsoft Azure. If it does, the function calls the Get-AzureVM cmdlet to get the VM. This function returns a hash table with vm and foundService keys.
...
.PARAMETER  ServiceName
The name of an existing Microsoft Azure service. This parameter is required.
...
Takes a hash table with name and password keys, such as: @{Name = "admin"; Password = "password"} This parameter is optional. If you omit it, the default values are the virtual machine user name and password in the JSON configuration file.
...
.PARAMETER SiteName
Specifies a name for the website. This parameter is required.

.PARAMETER Username
Specifies the user name of the website administrator. This parameter is required.

.PARAMETER Password
Specifies a password for the website administrator. Enter a password in plain text. Secure strings are not permitted. This parameter is required.

.PARAMETER AllowUntrusted
Allows untrusted SSL connections to the web deploy endpoint. This parameter is used in the call to MSDeploy.exe. This parameter is optional.
...
<#
.SYNOPSIS
Deploys a virtual machine to Microsoft Azure.
...
.PARAMETER  VMDnsName
Specifies the DNS name of the Microsoft Azure virtual machine. This parameter is required.
